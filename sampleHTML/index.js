...
CanvasRenderingContext2D.prototype.roundedRectangle = function (x, y, w, h, r) {
	...
	const handleUploadDocument = async () => {
		...
		if (editedSnap) {
			validateDocument(editedSnap,true)
		}
	}

	const handleUploadDocumentBackSide = async() => {
		debugger;
	...
	const handleCaptureDocument = async () => {
		...
		const editedSnap = await encodeImage(accessToken, snapshot);
		if (editedSnap) {
			validateDocument(editedSnap)
		}
	}

	const validateDocument = (editedSnap,isUpload = false) => {
		document.getElementById('loader').style.display = "block";
		getDocumentData(editedSnap).then((res) => {
			if (res.opDesc === "ok") {
				if (res.result.documentType === "passport" || res.result.documentType === "idCard" && !res.result.nextPage) {
					...
					const message = isUpload ? "Upload the opposite side of the document" : "Scan the opposite side of the document";
					Swal.fire(
						"Opposite Side",
						message,
						"info"
					...
					if(isUpload) {
						document.getElementById("content").style.display = "none";
						document.getElementById("UploadFirstPage").style.display = "none";
						document.getElementById("backSideDocContent").style.display = "block"
					}
				}
			} else {
				Swal.fire({
